...
HOME = os.getenv('HOME')
BASE_API_tccconnect_URL = 'https://api.tccconnectapp.com'
ASSETS_SCREENCONNECT_API = 'http://127.0.0.1/api/v1.1/assets'
...
def get_assets_by_connect_api():
    if click.confirm('Do you need authentication to access Connect API?'):
        ...
        auth = None
    response = requests.get(ASSETS_SCREENCONNECT_API, timeout=10, auth=auth)
...
def send_asset(asset):
    endpoint_url = f'{BASE_API_tccconnect_URL}/api/v4/assets'
    asset_uri = asset['uri']
    post_kwargs = {
        ...
        if asset['mimetype'] in ['image', 'video']:
            if asset_uri.startswith('/data'):
                asset_uri = os.path.join(
                    HOME, 'tccconnect_assets', os.path.basename(asset_uri))
