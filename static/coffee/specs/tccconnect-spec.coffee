...
describe "Screenconnect", ->
  ...
  it "should have a Screenconnect object at its root", ->
    expect(Screenconnect).toBeDefined()


  describe "date_to", ->

    test_date = new Date(2014, 5, 6, 14, 20, 0, 0);
    a_date = Screenconnect.date_to(test_date);
  ...
  describe "Collections", ->
    ...
    describe "Assets", ->
      it "should exist", ->
        expect(Screenconnect.Assets).toBeDefined()

      it "should use the Asset model", ->
        assets = new Screenconnect.Assets()
        expect(assets.model).toBe Screenconnect.Asset

      it "should keep play order of assets", ->
        assets = new Screenconnect.Assets()
        asset1 = new Screenconnect.Asset({
          asset_id: 1
          is_enabled: true
          name: 'AAA'
          uri: 'https://screenconnect.tccconnect.io',
  ...
  describe "Views", ->
    ...
    it "should have AddAssetView", ->
      expect(Screenconnect.View.AddAssetView).toBeDefined()

    it "should have EditAssetView", ->
      expect(Screenconnect.View.EditAssetView).toBeDefined()

    it "should have AssetRowView", ->
      expect(Screenconnect.View.AssetRowView).toBeDefined()

    it "should have AssetsView", ->
      expect(Screenconnect.View.AssetsView).toBeDefined()
